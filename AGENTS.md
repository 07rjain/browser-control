...
- Taught skills are local `SKILL.md` files under the sidebar Codex home `~/.codex-sidebar/skills`. The companion discovers that directory, shows each skill's name and description to the model, and loads a body only through the host `skills.read` tool. Skills outside that directory stay unavailable. Selecting a skill does not bypass permission mode or hard refusals. A user can record a demonstration from the sidebar; the recording stores role and accessible name, skips secrets and purchases, and writes a skill only after review.
