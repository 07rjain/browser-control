...
Browser tools are independently schema-validated and executed by the service worker. `tabs.group` accepts only existing unpinned tab IDs from one window and uses the typed Chrome Tabs/Tab Groups APIs. `tabs.ungroup` removes grouping without closing tabs. `tabs.close` pauses for an explicit side-panel confirmation in Ask every time mode and executes directly in the default Full access mode. Only `http` and `https` URLs can be opened.
