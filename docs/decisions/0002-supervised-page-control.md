...
- Require optional host permission for the exact active `http` or `https` origin before any `page.*` execution. Permission is requested only from a sidebar user gesture and revoked when the browser task ends.
...
- Generate opaque element references inside the executor. Bind references to snapshot, tab, and origin; expire them after 30 seconds, mutation, navigation, origin change, or task completion.
- Allow ordinary same-origin links automatically. Confirm buttons, external/new-tab links, downloads, Enter, tab close, and form submission. Refuse purchases, financial actions, secrets, CAPTCHAs, and security bypasses.
...
- Maximum 80 inspected elements, 30-second reference lifetime, eight-second waits, and 20 page actions per turn.
...
- No `debugger` permission, CDP, arbitrary JavaScript, remotely hosted code, background remote control, downloads, or connectors.

## Consequences

The approach is less capable than coordinate or debugger automation, but it is auditable, permission-scoped, compatible with Chrome Web Store security expectations, and resilient against model attempts to invent executable selectors. Dynamic applications can invalidate references frequently and require re-inspection. Some custom widgets and untrusted keyboard defaults cannot be controlled reliably and must return an error instead of claiming success.
